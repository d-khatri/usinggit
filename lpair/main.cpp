...
long long int mergeSort(long long int arr[], long long int siz)
{
    long long int *temp = new long long int[siz];
    return _mergeSort(arr, temp, 0, siz - 1);
}
...
int main()
{
    long long int n,i,j,*arry,*arry2;
    cin>>n;
    arry=new long long int [n];
    arry2=new long long int [n];
    for(i=0;i<n;i++){
        cin>>arry2[i]>>arry[i];
    }

    cout<<mergeSort(arry,n);
    return 0;
}
